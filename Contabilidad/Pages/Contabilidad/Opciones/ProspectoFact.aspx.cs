...
public partial class Pages_Opciones_ProspectoFact : System.Web.UI.Page
{
    ...
    protected void txtProspecto_TextChanged(object sender, EventArgs e)
    {
        ...
        if (obj != null)
        {
            ...
            txtCP.Text = obj.strKeys.ToString();
            txtDescuento.Text = obj.DblDescuento.ToString();
            txtEmail.Text = obj.StrEmail;
            txtNombre.Text = obj.StrProspecto;
            txtNombreCliente.Text = obj.StrNombreCliente;
            txtApellidoPaterno.Text = obj.StrApellidoPaterno;
            txtApellidoMaterno.Text = obj.StrApellidoMaterno;
            ...
            if (obj.Constancia == "1")
            {
                ...
                if(obj.IntDocumento > 0)
                    btnPreview.CommandArgument = obj.IntDocumento.ToString();
                else
                    btnPreview.Visible = false;
            }
            else
            {
                btnPreview.CommandArgument = "";
                btnPreview.Visible = false;
            }

            cboTipoPersona.SelectedValue = obj.IntTipoRegimen.ToString();
            cboTipoPersona_SelectedIndexChanged(cboTipoPersona,null);
            cboRegimen.SelectedValue = obj.IntRegimenFiscal.ToString();
            txtNumero.Text = obj.StrNumero;
            txtNumeroExt.Text = obj.StrNumeroExt;

            btnSave.UpdateAfterCallBack = true;
            btnPreview.UpdateAfterCallBack = true;
            cboTipoPersona.UpdateAfterCallBack = true;
            cboRegimen.UpdateAfterCallBack = true;
            txtNombreCliente.UpdateAfterCallBack = true;
            txtApellidoPaterno.UpdateAfterCallBack = true;
            txtApellidoMaterno.UpdateAfterCallBack = true;
            txtNumero.UpdateAfterCallBack = true;
            txtNumeroExt.UpdateAfterCallBack = true;
    ...
    {
        //REGIMEN FISCAL
        int intTipo = Convert.ToInt32(cboTipoPersona.SelectedValue);
        DataTable dt;
        ProspectoEsc obj = new ProspectoEsc();
        dt = obj.RegimenFiscal(intTipo);
        cboRegimen.DataSource = dt;
        cboRegimen.DataTextField = "strNombre";
        cboRegimen.DataValueField = "intRegimen";
        cboRegimen.DataBind();

        cboRegimen.Items.Insert(0, new System.Web.UI.WebControls.ListItem("--- Seleccione ---", "0"));
        cboRegimen.UpdateAfterCallBack = true;
        //REGIMEN FISCAL
    }
    ...
    [WebMethod]
    public static string[] SaveFormaPago(int intFactura, string strMetodoPago, string strFormaPago, string strUsoCFDI)
    {
        string[] rtnData = new string[2];
        try
        {
            ProspectoEsc obj = new ProspectoEsc();
            obj.SaveFormaPago(intFactura, strMetodoPago, strFormaPago, strUsoCFDI);
    ...
    [WebMethod]
    public static string[] GenerarFacturas(string intProspecto, string strFolio)
    {
        string[] rtnData = new string[2];
        int intFolio = 0;
        ...
        try
        {
            ...
            foreach (string crtName in arrCampos)
            {
                ...
                obj.SaveGenerarXFaC(intPros, intFolio);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            obj.IntProspecto = Convert.ToInt32(txtProspecto.Text);
            obj.StrPais = txtPais.Text;
            obj.StrEstado = txtEstado.Text;
            obj.StrCiudad = txtCiudad.Text;
            obj.StrColonia = txtColonia.Text;
            obj.StrCalle = txtCalle.Text;
            obj.StrNumero = txtNumero.Text;
            obj.StrNumeroExt = txtNumeroExt.Text;
            obj.strKeys = txtCP.Text;
            obj.StrTelefono = txtTelefono.Text;
            obj.StrEmail = txtEmail.Text;
            obj.StrRFC = txtRFC.Text;
            obj.IntRegimenFiscal = Convert.ToInt32(cboRegimen.SelectedValue);
            obj.StrNombreCliente = txtNombreCliente.Text;
            obj.StrApellidoPaterno = txtApellidoPaterno.Text;
            obj.StrApellidoMaterno = txtApellidoMaterno.Text;

            pros.SaveDatosFacturacion(obj);
            txtProspecto_TextChanged(null, null);

            Anthem.Manager.RegisterStartupScript(Page.GetType(), "Savew23", "alert('Se actualizaron los datos de facturacion.');", true);
        ...
        {
            Anthem.Manager.RegisterStartupScript(Page.GetType(), "er2r", "alert('" + ex.Message + "');", true);
        }
    }
    #endregion
