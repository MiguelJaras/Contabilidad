...
namespace Contabilidad.DataAccess
{
    public class DACSector : Base
    {
        ...
        #endregion
        public static string GetSectores(string strObras)
        {
            string result = "";
            ...
            try
            {
                ...
                string query = @"DECLARE @Colonias VARCHAR(300) SELECT  @Colonias = COALESCE(@Colonias + ', ', '') + strNombre FROM tbSector WHERE intSector IN(SELECT intSector FROM tbObra where intObra in(" + strObras + ")) SELECT @Colonias";
