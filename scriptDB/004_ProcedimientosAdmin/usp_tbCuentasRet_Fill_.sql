IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'dbo.usp_tbCuentasRet_Fill_')
	AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
BEGIN
	DROP PROCEDURE dbo.usp_tbCuentasRet_Fill_
	PRINT N'Drop Procedure : dbo.usp_tbCuentasRet_Fill_ - Succeeded !!!'
END
GO
...
------------------------------------------------------------------------------------
CREATE PROCEDURE usp_tbCuentasRet_Fill_
(
	@intEmpresa					INT,
	@intArea					INT,
...
AS
 ...
 INNER JOIN tbCuentas CtaAbono
	  ...
	  WHERE CR.intEmpresa = CASE ISNULL(@intEmpresa,0) WHEN 0 THEN CR.intEmpresa ELSE ISNULL(@intEmpresa,0) END
		AND CR.intArea		= @intArea
		AND CR.strInsumoIni = @strInsumoIni
		AND CR.strInsumoFin = @strInsumoFin
		AND CR.strCuentaCargo = @strCuentaCargo
		AND CR.strCuentaAbono = @strCuentaAbono
...
IF (@@Error = 0)
BEGIN
	PRINT 'Procedure Creation: dbo.usp_tbCuentasRet_Fill_ Succeeded'
END
ELSE
BEGIN
	PRINT 'Procedure Creation: dbo.usp_tbCuentasRet_Fill_ Error on Creation'
END
GO